...
resource "google_compute_network" "gcp-network" {
  name                    = "${var.hiring_manager_name}-sa-candidate-${var.candidate_name}-network"  # "gcp-vm-${var.gcp_region}"
  auto_create_subnetworks = "false"
}

resource "google_compute_subnetwork" "gcp-subnet1" {
  name          = "${var.hiring_manager_name}-sa-candidate-${var.candidate_name}-subnet1"
